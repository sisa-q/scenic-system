...
let deltaSync = null
let hud = null
// ===== 双数据引擎：'sim' 模拟引擎（默认） / 'real' 实时引擎（真实统计接口 + WebSocket 推送） =====
let dataSource = 'sim'
let realFlow = null
let realStats = null
let realWs = null
let realPollTimer = null
let realStatsTimer = null
let statsLastSource = 'sim'
// 实时引擎缩放系数：真实在园/模拟在园（0.25~4），驱动热力/等值线/柱体/粒子
let realScale = 1
// 后端 WebSocket 地址（默认本机 8083，可用 VUE_APP_WS_URL 覆盖）
const WS_FLOW_URL = process.env.VUE_APP_WS_URL || ((location.protocol === 'https:' ? 'wss://' : 'ws://') + location.host + '/ws/flow')
...
let statsPanel = null
let statInpark = null
let statTodayEntered = null
let statTodayOrders = null
let statCumulative = null
let statPeak = null
let statBars = null
...
export function initScene(containerElement) {
    ...
    scene.fog = new THREE.Fog(0x0a0a2a, 28, 90)

    camera = new THREE.PerspectiveCamera(42, width / height, 0.05, 200)
...
// 金色屋顶材质：轻微自发光 + 双面渲染，保证任意角度/任意光照下都清晰可见
function roofMat(roughness, metalness) {
    return new THREE.MeshStandardMaterial({ color: MAT_COLORS.roof, roughness, metalness, emissive: 0xffcc55, emissiveIntensity: 0.35, side: THREE.DoubleSide })
}
...
function buildHighLOD(dim) {
    ...
    // 檐椽（出挑）
    const eave = new THREE.Mesh(new THREE.BoxGeometry(w + 0.24, 0.03, d + 0.24), roofMat(0.45, 0.25))
    ...
    // 翼角（四角出挑，示意翼角椽飞）
    if (dim.grand) {
        const wingMat = roofMat(0.45, 0.3)
    ...
    const roofH = h * 0.30
    const roof = new THREE.Mesh(new THREE.ConeGeometry(roofR, roofH, 8), roofMat(0.4, 0.35))
    ...
    if (dim.doubleEave) {
        ...
        const roof2H = h * 0.20
        const roof2 = new THREE.Mesh(new THREE.ConeGeometry(roof2R, roof2H, 8), roofMat(0.4, 0.35))
    ...
    // 正脊 + 宝顶
    const ridge = new THREE.Mesh(new THREE.BoxGeometry(Math.max(w, d) * 0.45, 0.02, 0.03), roofMat(0.4, 0.3))
...
function buildMidLOD(dim) {
    ...
    const roofH = h * 0.36
    const roof = new THREE.Mesh(new THREE.ConeGeometry(roofR, roofH, 8), roofMat(0.4, 0.3))
    roof.position.y = 0.05 + wallH + roofH / 2 + 0.01
    roof.rotation.y = Math.PI / 8
    roof.castShadow = true
    g.add(roof)
    const ridge = new THREE.Mesh(new THREE.BoxGeometry(Math.max(w, d) * 0.4, 0.02, 0.03), roofMat(0.4, 0.3))
...
function createGugongBuildings() {
    ...
    ids.forEach((id, index) => {
        ...
        mid.visible = false
        // 任意旋转角度/任意缩放均完整显示：关闭视锥体裁剪 + 双面渲染
        high.traverse((o) => { if (o.isMesh) { o.frustumCulled = false; o.material.side = THREE.DoubleSide } })
        mid.traverse((o) => { if (o.isMesh) { o.frustumCulled = false; o.material.side = THREE.DoubleSide } })
        root.add(high)
        root.add(mid)
...
function createCityWall() {
    const wallMat = new THREE.MeshStandardMaterial({ color: 0x8B4513, roughness: 0.9, metalness: 0.0, side: THREE.DoubleSide })
    ...
    // 四角角楼
    const towerMat = new THREE.MeshStandardMaterial({ color: 0x8B4513, roughness: 0.8, side: THREE.DoubleSide })
    ;[[xMin, zMin], [xMax, zMin], [xMax, zMax], [xMin, zMax]].forEach(([tx, tz]) => {
        ...
        scene.add(tower)
        const roof = new THREE.Mesh(new THREE.ConeGeometry(0.24, 0.22, 8), roofMat(0.4, 0.3))
...
function applyStats(data) {
    ...
    realStats = data
    if (dataSource === 'real') {
        statsLastSource = null // 强制重绘统计面板
        updateStatsPanel()
    }
}

/** 启动实时引擎：WebSocket 推送 + 轮询兜底 + 完整统计 */
function startRealEngine() {
    if (realWs || realPollTimer) return
    // 1) WebSocket：后端 /ws/flow 每 5 秒推送真实客流
    try {
        realWs = new WebSocket(WS_FLOW_URL)
        realWs.onmessage = (e) => {
            try { applyRealtime(JSON.parse(e.data)) } catch (err) { /* 忽略非法帧 */ }
        }
        realWs.onerror = () => { if (realWs) { try { realWs.close() } catch (e) { /* 忽略 */ } } }
        realWs.onclose = () => { realWs = null }
    } catch (err) {
        realWs = null
    }
    // 2) 轮询兜底：WebSocket 不可用时仍能拿到真实数据
    const poll = async () => {
        try {
            const res = await getRealTime()
            applyRealtime(res && res.data ? res.data : null)
        } catch (e) { /* 后端暂不可用则保留上一帧 */ }
    }
    poll()
    realPollTimer = setInterval(poll, 5000)
    // 3) 完整统计（今日订单 / 累计游客 / 时段占比等）
    const fetchStats = async () => {
        try {
            const res = await getFlowStats()
            applyStats(res && res.data ? res.data : null)
        } catch (e) { /* 忽略 */ }
    }
    fetchStats()
    realStatsTimer = setInterval(fetchStats, 30000)
}

/** 停止实时引擎 */
function stopRealEngine() {
    if (realWs) { try { realWs.close() } catch (e) { /* 忽略 */ } realWs = null }
    if (realPollTimer) { clearInterval(realPollTimer); realPollTimer = null }
    if (realStatsTimer) { clearInterval(realStatsTimer); realStatsTimer = null }
    realFlow = null
    realStats = null
    realScale = 1
}

/** 切换数据源：'sim' 模拟 / 'real' 实时 */
function switchDataSource(mode) {
    const next = mode === 'real' ? 'real' : 'sim'
    ...
    statsLastSource = null
    updateStatsPanel()
    updateHUD()
}

/** 重绘"各时段入园占比"条形图（模拟传 0..1 比例数组；实时传 value 数组） */
function renderHourlyBars(values) {
    if (!statBars) return
    statBars.innerHTML = ''
    const arr = values || []
    ...
    arr.forEach((v, k) => {
        ...
        fill.style.cssText = 'height:100%;width:' + (Math.max(0, v) / max * 100) + '%;background:linear-gradient(to right,#4a9eff,#4dffa6);border-radius:4px'
        track.appendChild(fill)
        const pct = document.createElement('span')
        pct.textContent = (v / max * 100).toFixed(1) + '%'
        pct.style.cssText = 'width:46px;flex:none;color:#cfe8ff'
        hrow.appendChild(lab)
        hrow.appendChild(track)
        hrow.appendChild(pct)
        statBars.appendChild(hrow)
    })
}
...
function createControlButtons() {
    ...
    btnSource.textContent = '数据源 模拟'
    btnSource.onclick = () => {
        const next = dataSource === 'sim' ? 'real' : 'sim'
        switchDataSource(next)
        btnSource.textContent = '数据源 ' + (dataSource === 'sim' ? '模拟' : '实时')
        btnSource.style.borderColor = dataSource === 'real'
                ? 'rgba(77,255,166,0.85)'
                : 'rgba(120,180,255,0.35)'
    }

    ;[btnFull, btnQuality, btnSource].forEach((btn) => {
    ...
    hudButtons = { btnQuality, btnSource }
}
...
function createStatsPanel() {
    ...
    row2.innerHTML = '<span>今日入园 <b id="gg-todayentered" style="color:#ffd76a">' + fmtNum(summary.totalEntries) + '</b></span>'
    statTodayEntered = row2.querySelector('#gg-todayentered')
    const row3 = document.createElement('div')
    row3.style.cssText = 'display:flex;justify-content:space-between;color:#9fb8d8;font-size:12px'
    row3.innerHTML = '<span>今日订单 <b id="gg-todayorders" style="color:#ffd76a">' + fmtNum(summary.totalOrders) + '</b></span>'
    statTodayOrders = row3.querySelector('#gg-todayorders')
    const row4 = document.createElement('div')
    row4.style.cssText = 'display:flex;justify-content:space-between;color:#9fb8d8;font-size:12px'
    row4.innerHTML = '<span>累计游客 <b id="gg-cumulative" style="color:#ffd76a">' + fmtNum(summary.cumulativeVisitors) + '</b></span>'
    statCumulative = row4.querySelector('#gg-cumulative')
    const row5 = document.createElement('div')
    row5.style.cssText = 'display:flex;justify-content:space-between;color:#9fb8d8;font-size:12px'
    row5.innerHTML = '<span>峰值在园 <b id="gg-peak" style="color:#ffd76a">' + fmtNum(summary.peakInPark) + '</b></span>'
    statPeak = row5.querySelector('#gg-peak')
    ...
    statBars = bars
    renderHourlyBars(summary.hourlyProportion)
...
function updateStatsPanel() {
    ...
    if (dataSource === 'real') {
        // ===== 实时引擎：真实统计接口 + WebSocket 推送 =====
        statInpark.textContent = fmtNum(realFlow ? realFlow.currentVisitors : 0)
        if (realFlow && statTodayEntered) statTodayEntered.textContent = fmtNum(realFlow.todayEntered)
        if (realStats) {
            if (statTodayOrders) statTodayOrders.textContent = fmtNum(realStats.todayOrders)
            if (statCumulative) statCumulative.textContent = fmtNum(realStats.totalVisitors)
            if (statPeak) statPeak.textContent = '--'
        } else {
            if (statTodayOrders) statTodayOrders.textContent = '--'
            if (statCumulative) statCumulative.textContent = '--'
            if (statPeak) statPeak.textContent = '--'
        }
        if (statsLastSource !== 'real') {
            statsLastSource = 'real'
            renderHourlyBars(realStats && realStats.hourlyDistribution
                    ? realStats.hourlyDistribution.map(h => h.value) : [])
        }
    } else {
        // ===== 模拟引擎 =====
        ...
        if (statsLastSource !== 'sim') {
            ...
            if (statPeak) statPeak.textContent = fmtNum(summary.peakInPark)
            renderHourlyBars(summary.hourlyProportion)
        }
    }
}
