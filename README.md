...
- **🧠 本地大模型智能体（核心差异化）**：Ollama 本地部署 Qwen2.5-3B（数据不出内网）（微调原始数据JSONL入库（docs/dataset）（3050显卡4G显存限制，由Qwen2.5-3B Q4_K_M 单模型 + CPU 嵌入 + 意图路由串行 + 3D 走核显 + 上下文瘦身 + base 模型工具链）；意图路由分发**游客服务 / 运营决策 / 应急调度**三类智能体；Function Calling 调用 **10 个工具**（查票/下单/支付/退款…）；RAG 知识库检索降低幻觉；危险操作**确认卡 = 人在回路**；AI 悬浮窗支持**四步自动购票链**过程可视化
