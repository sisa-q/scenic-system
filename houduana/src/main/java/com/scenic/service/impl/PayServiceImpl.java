...
public class PayServiceImpl implements PayService {
    ...
    /**
     * 手动“确认支付结果”（沙箱模式闭环）：对待支付订单主动查询支付宝交易状态，
     * 已支付则按模拟支付逻辑确认订单。不依赖异步通知/隧道，后端直连支付宝网关。
     */
    @Override
    public PayResult refreshOrderPaymentStatus(Long orderId, Long operatorId, String role) {
        ...
        // 只有待支付订单才查询支付宝
        if (order.getStatus() != null && order.getStatus() == 0 && alipaySigner != null && payProperties != null) {
            try {
                ...
                if ("10000".equals(respNode.path("code").asText())
                    ...
                    log.info("[Pay][refresh] 手动确认结果: {}", result);
                }
            } catch (Exception e) {
