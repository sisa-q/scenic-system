...
public class PayServiceImpl implements PayService {
    ...
    public String handleNotify(Map<String, String> params) {
        try {
            ...
            log.info("[Pay][notify] 收到异步通知: out_trade_no={}, trade_status={}, total_amount={}, hasSign={}",
            ...
            if (realAlipay() && alipaySigner != null && payProperties != null) {
                ...
                boolean ok = sign != null && alipaySigner.verifyNotify(params, sign, payProperties.getAlipayPublicKey());
                log.info("[Pay][notify] 验签结果: {}", ok);
                if (!ok) {
                    return FAILURE;
                }
            }
            ...
            if (!"TRADE_SUCCESS".equals(tradeStatus) && !"TRADE_FINISHED".equals(tradeStatus)) {
                log.warn("[Pay][notify] 非成功状态，忽略: {}", tradeStatus);
                return FAILURE;
            }
            String result = confirmPaidOrder(params.get("out_trade_no"), params.get("trade_no"), params.get("total_amount"));
            log.info("[Pay][notify] 确认结果: {}", result);
            return result;
        } catch (Exception e) {
            log.warn("[Pay][notify] 处理异常: {}", e.getMessage());
            return FAILURE;
        }
    }
     ...
     * 用于异步通知延迟/丢失时，保证支付成功的订单也能变为“已支付”。
     */
    @Override
    public PayResult refreshOrderPaymentStatus(Long orderId, Long operatorId, String role) {
        ...
        // 只有待支付订单才主动查询支付宝（已支付/已退款/已关闭等无需查询）
